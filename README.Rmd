...
skip_vignette <- !reticulate::py_module_available("real_ladybug")
```
...
#> The 'real_ladybug' Python package is installed and available.
```
...
```{r example, message=FALSE, warning=FALSE, fig.show='hold', fig.cap="Plot of the graph structure created from Ladybug query results.", eval=!skip_vignette}
library(lbugr)
library(igraph)
library(tidygraph)
...
- For more detailed examples on how to use Ladybug Query see [Ladybug documentation](https://docs.ladybugdb.com/)
